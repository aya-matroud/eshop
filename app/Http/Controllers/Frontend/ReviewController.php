...
class ReviewController extends Controller
{
    public function add($product_slug){
        ...
        if($product){
            ...
            if($review){
                return view('layouts.frontend.reviews.edit',compact('review'));
            }
            else{
                $verified_purchase=Order::where('orders.user_id',Auth::id())
                    ->join('order_items','orders.id','order_items.order_id')
                    ->where('order_items.prod_id',$product_id)->get();
                return view('layouts.frontend.reviews.index',compact('product','verified_purchase'));
            }
    ...
    public function create(Request $request)
    {
        ...
        if ($product) {
            ...
            if ($new_review) {
                return redirect('category/' . $category_slug . '/' . $product_slug)->with('status', "Thank u for writing a review");
            }
